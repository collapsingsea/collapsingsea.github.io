// script.js (Complete Code - 3-Column Resizable Layout & Auto-Send)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderChatLog(history) {
        ...
        if (!history || history.length === 0) {
             addMessage('ai', '你好！开始提问吧。'); // Welcome for empty chat
             return;
        }
    ...
    function addMessage(sender, content, isStreaming = false, searchResults = null) {
        ...
        if (sender === 'ai' && typeof content === 'string' && content.startsWith('错误:')) messageDiv.classList.add('error'); // Add error class based on content
    ...
    async function processAndSendMessage(textContent, imageFileParts = [], isAutoSend = false) {
        if (!currentChatId) { console.error("Cannot send message: No active chat."); return; }
        const currentChat = allChats.find(c => c.id === currentChatId);
        if (!currentChat) { console.error("Cannot send message: Current chat not found!"); return; }

        const userParts = [];
        if (textContent) userParts.push({ text: textContent });
        userParts.push(...imageFileParts);
        if (userParts.length === 0) { console.warn("Attempted to send empty message."); return; }
        ...
        try {
            await streamApiResponse(
             ...
             // Catch errors re-thrown by streamApiResponse (e.g., fetch errors)
    ...
    function startAutoSend() {
        ...
        if (isAutoSending) {
            ...
            // If the first send failed and stopped auto-sending, reset the UI
            stopAutoSend();
        }
    }
    ...
    function makeResizable(resizerElement, leftElement, rightElement) {
        ...
        const onMouseMove = (e) => {
            ...
            // Enforce minimum widths, prioritize the element hitting the minimum
             if (newLeftWidth < MIN_SIDEBAR_WIDTH) {
                 newLeftWidth = MIN_SIDEBAR_WIDTH;
                 ...
                 if (newLeftWidth < MIN_SIDEBAR_WIDTH) newLeftWidth = MIN_SIDEBAR_WIDTH;
             }
             ...
             if (newLeftWidth + newRightWidth > startLeftWidth + startRightWidth) {
                 // This shouldn't normally happen with the logic above, but as a fallback:
                 // Prioritize the direction of movement, adjust the other column
            ...
            // Only set width directly for the right sidebar, center uses flex-grow
            if (rightElement === settingsSidebar) {
                 rightElement.style.width = `${newRightWidth}px`;
            } else if (leftElement === centerContent) {
                // Special case: Dragging right resizer sets center width directly
                 leftElement.style.width = `${newLeftWidth}px`;
            }
             // else: Center column adjusts via flex-grow when left resizer moves
        };
    ...
    // 7. Focus input
    userInput.focus();
    console.log("Chat interface initialized (Resizable 3-column).");
